"""Perplexity provider — Sonar models via OpenAI-compatible API."""
...
PERPLEXITY_MODELS: dict[str, tuple[float, float]] = {
    "sonar":        (1.00,  1.00),
...
def _cost(model: str, input_tokens: int, output_tokens: int) -> float:
    prices = PERPLEXITY_MODELS.get(model)
    if not prices:
        return 0.0
    return round(
...
class PerplexityProvider(BaseProvider):
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        if not self._api_key:
            logger.warning("PERPLEXITY_API_KEY not set — PerplexityProvider will return stubs")
    ...
    async def complete(self, request: CompletionRequest) -> CompletionResponse:
        if not self._api_key:
            return CompletionResponse(
                content="[Perplexity unavailable: no API key]",
                model=request.model or _DEFAULT_MODEL,
        ...
        try:
            ...
            self.record_error()
            logger.error("PerplexityProvider.complete error: %s", exc)
            raise

    async def stream(self, request: CompletionRequest) -> AsyncIterator[str]:  # type: ignore[override]
        if not self._api_key:
            yield "[Perplexity unavailable: no API key]"
            return
        ...
        try:
            ...
            self.record_error()
            logger.error("PerplexityProvider.stream error: %s", exc)
            raise
