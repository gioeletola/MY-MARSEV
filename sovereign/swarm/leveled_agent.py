...
from sovereign.router.model_router import ModelRouter as _ModelRouter, RoutingCriteria as _RoutingCriteria
...
def _make_leveled_worker(
    ...
    async def execute(self, task: AgentTask, ctx: AgentContext, plan: dict) -> dict:
        ...
        _complexity = _ModelRouter.estimate_complexity(task.objective)
        _routed_model = _router.route(_RoutingCriteria(
            task_complexity=_complexity,
            action_class=str(task.action_class) if task.action_class else "SUGGEST",
            requires_reasoning=_complexity >= 0.7,
        ))
        prompt = (
        ...
        try:
            if _lvl in (AgentLevel.LEVEL_3, AgentLevel.LEVEL_4):
                ...
                result, history = await self._claude.complete_with_tool_loop(
                ...
                for block in resp.content:
                    if hasattr(block, "text"):
                        text += block.text
                return {"result": text, "tool_turns": 0, "mode": "single_call", "model_used": _routed_model}
        except Exception as exc:
    ...
    async def report(self, task: AgentTask, ctx: AgentContext, all_steps: dict) -> StructuredOutput:
        ...
        out = self._make_output(
            ...
            data={
                ...
                "model_used": exec_data.get("model_used", _model),
