...
def _make_worker(
    ...
    def _run(self, task: AgentTask, ctx: AgentContext) -> StructuredOutput:
        raise NotImplementedError  # replaced by async version below

    async def _async_run(self, task: AgentTask, ctx: AgentContext) -> StructuredOutput:
        import logging
        ...
        try:
            ...
            prompt = (
                ...
                f"{instructions}\n\n"
                f"Task:\n{task.objective}\n\n"
                "Respond with precision, strategic depth, and clear action recommendations."
            )
